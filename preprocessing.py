...
def generate_encodec_tokens(model, processor, dataset, device, output_dir, embed_dim=768, sample_rate=24000):
    ...
    # Optional: single embedding layer to demonstrate how to project them
    projection_layer = nn.Embedding(
        num_embeddings=1024,  # each codebook index in [0..1023]
...
def run_preprocessing(dataset, cfg, device):
    """
    Main entry to run MERT and Encodec preprocessing.
    """
    if cfg['run_mert']:
